define({
    ...
    plsWork: function() {
        kony.print("entered login controler");
        var authMode = kony.mvc.MDAApplication.getSharedInstance().getModuleManager().getModule("AuthModule");
        authMode.presentationController.dummy();
    },
    ...
    animateFlxContent: function() {
        this.view.flxContent.animate(kony.ui.createAnimation({
            "100": {
                "anchorPoint": {
                    "x": 0.5,
                    "y": 0.5
    ...
    resetLoginUI: function() {
        this.view.flxAccountPreview.top = "20%";
        this.view.flxContent.top = "27.1%";
    ...
    manageUname: function(loginData) {
        if (loginData.isRememberMeOn != true) {
            ...
            if (loginData.isFirstTimeLogin != true) this.view.tbxUsername.text = loginData.userName;
            else this.view.tbxUsername.text = "";
            this.view.tbxPassword.text = "";
    ...
    statusCB: function(status, msg) {
        if (status == 5000) this.doLoginonTouchId();
        this.view.flxWelcome.setEnabled(true);
        this.view.flxContent.setEnabled(true);
        this.view.flxFooter.setEnabled(true);
    },
    authCallBack: function(status, msg) {
        ...
        } else applicationManager.getDataProcessorUtility().showToastMessageError(this, "Please Try Again with Valid Fingerprint");
        this.view.flxWelcome.setEnabled(true);
        this.view.flxContent.setEnabled(true);
        this.view.flxFooter.setEnabled(true);
    },
    ...
    frmLoginPreShow: function() {
        ...
        this.adsPreshow();
        if (authMode.presentationController.isappInitDone()) applicationManager.getPresentationUtility().dismissLoadingScreen();
        var navManager = applicationManager.getNavigationManager();
        var currentForm = navManager.getCurrentForm();
    ...
    rememberMeOption: function() {
        ...
        if (rememberMeflag == "remembermetick.png") {
            this.view.imgCheckBox.src = "remeberme.png";
            if (loginData.istouchIdEnabled || loginData.isPinModeEnabled || loginData.isFacialAuthEnabled) this.showTouchIdOffAlert(applicationManager.getPresentationUtility().getStringFromi18n("kony.mb.rememberMe.Msg"), applicationManager.getPresentationUtility().getStringFromi18n("kony.mb.rememberMeTittle"));
    ...
    faceIdVerify: function() {
        var authMod = kony.mvc.MDAApplication.getSharedInstance().getModuleManager().getModule("AuthModule");
        authMod.presentationController.FaceAuthVerify();
        this.gemaltoTimerVar = false;
    },
