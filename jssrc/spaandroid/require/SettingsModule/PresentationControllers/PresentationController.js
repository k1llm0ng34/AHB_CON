define([], function() {
    ...
    Settings_PresentationController.prototype.getUserAllPhoneNumbersSuccess = function(data) {
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            } else if (data[i].countryType.toLowerCase() === 'international') {
                name += '(International)';
            }
            temp.lblDetail = name;
    ...
    Settings_PresentationController.prototype.navigateToEditPhoneNumber = function(info) {
        ...
        for (var i = 0; i < phoneNumbers.length; i++) {
            ...
            } else if (phoneNumbers[i].countryType.toLowerCase() === "international") {
    ...
    Settings_PresentationController.prototype.deRegisterDevice = function() {
        applicationManager.getPresentationUtility().showLoadingScreen();
        var registrationManager = applicationManager.getRegistrationManager();
        var deviceUtilManager = applicationManager.getDeviceUtilManager();
        var record = {
        ...
        registrationManager.deleteRegisteredDevice(record, this.deleteRegSuccess, this.deleteRegError);
    };
    Settings_PresentationController.prototype.deleteRegSuccess = function(success) {
        applicationManager.getPresentationUtility().dismissLoadingScreen();
        var authMod = kony.mvc.MDAApplication.getSharedInstance().getModuleManager().getModule("AuthModule");
        var userObj = applicationManager.getUserPreferencesManager();
        userObj.updateAccountPreviewFlag(false);
        if (userObj.isFaceEnrolled()) authMod.presentationController.FaceAuthUnenroll();
    ...
    Settings_PresentationController.prototype.updatePinSuccess = function(success) {
        ...
        var navManger = applicationManager.getNavigationManager();
        var msgData = {
            popUpMsg: "Login Pin has been changed successfully."
        };
    ...
    Settings_PresentationController.prototype.updateEmail = function(data) {
        ...
        if (index === 1) {
            ...
            if (data.isPrimary === 1) {
                ...
                if (oldSecondaryEmail !== null && oldSecondaryEmail !== undefined && oldSecondaryEmail !== "") jsonParams.secondaryemail = primaryEmail;
                else {
                    jsonParams.secondaryemail2 = primaryEmail;
                }
                userObj.partialUpdateOnUserObj(jsonParams, successCallback, failureCallback);
            } else {
                userObj.partialUpdateOnUserObj({
                    whichEmail: data.email
    ...
    Settings_PresentationController.prototype.deleteEmail = function(index) {
        ...
        if (index === 2) {
            applicationManager.getPresentationUtility().showLoadingScreen();
            userManager.partialUpdateOnUserObj({
                "secondaryemail2": ""
            }, successCallback, failureCallback);
        }
    ...
    Settings_PresentationController.prototype.enrollFaceId_Settings = function() {
        intializeFacialAuth();
        FaceAuth_initializeSettings();
    };
    Settings_PresentationController.prototype.enrollFaceId_OnSuccess = function() {
        ...
        var flags = {
            popUpMsg: kony.i18n.getLocalizedString("kony.mb.devReg.faceIdSucMsg")
        };
