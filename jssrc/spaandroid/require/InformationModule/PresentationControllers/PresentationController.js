define([], function() {
    ...
    Information_PresentationController.prototype.onClickPrivacyPolicy = function(headerValue) {
        ...
        function presentationSuccessCallback(response) {
            ...
            navManager.setCustomInfo("frmSupportInfo", {
                "richTextData": response.records[0].Description,
                "header": headerValue
            });
        ...
        function presentationErrorCallback(response) {
            ...
            if (response["isServerUnreachable"]) applicationManager.getPresentationInterruptHandler().showErrorMessage("preLogin", response);
        }
    }
    ...
    Information_PresentationController.prototype.onClickAboutUs = function(headerValue) {
        ...
        function presentationSuccessCallback(response) {
            var navManager = applicationManager.getNavigationManager();
            navManager.setCustomInfo("frmSupportInfo", {
                "richTextData": response[0].infoContent,
                "header": headerValue
            });
            navManager.navigateTo("frmSupportInfo");
        }
    ...
    Information_PresentationController.prototype.onClickFAQs = function(headerValue) {
        ...
        function presentationSuccessCallback(response) {
            ...
            for (var i = 0; i < faqsData.length; i++) {
                var nestedfaqs = faqsData[i].faqs;
                for (var j = 0; j < nestedfaqs.length; j++) {
                    arrayofFAQs.push(nestedfaqs[j]);
                }
            }
            navManager.setCustomInfo("frmSupportInfo", {
                "richTextData": arrayofFAQs,
                "header": headerValue
            });
            navManager.navigateTo("frmSupportInfo");
        }
