...
define([], function() {
    ...
    NavigationManager.prototype.navigateTo = function(formname) {
        ...
        if (index == null) this.setFormIndex(formname, stackLength - 1);
        else {
    ...
    NavigationManager.prototype.goBack = function(navDetails) {
        ...
        if (stackLength !== 0) {
            ...
            if (navDetails) existingNavDetails = this.getCustomInfo(formId);
    ...
    NavigationManager.prototype.destroyForms = function() {
        for (var i = 0; i < this.formStack.length; i++) {
            if (this.formStack[i] !== "frmLogin") kony.application.destroyForm(this.formStack[i]);
        }
        this.formStack = [];
