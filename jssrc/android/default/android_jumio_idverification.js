        ...
        function _initialize() {
            try {
                if (config.initParams.offlineToken) {
                    if (config.initParams.preferredCountry === undefined) config.initParams.preferredCountry = null;
            ...
            if (!netverifySDK) {
                ...
                for (var index in missval) {
                    ...
                    if (index < missval.length - 1) errmsg += ", ";
                }
                ...
                for (var param in config[category]) {
                    switch (param) {
                        case "requireVerification":
                            netverifySDK.setRequireVerification(config[category][param]);
                            break;
                        case "callbackURL":
                            netverifySDK.setCallbackUrl(config[category][param]);
                            break;
                        case "requireFacematch":
                            netverifySDK.setRequireFaceMatch(config[category][param]);
                            break;
                        case "dataExtractionOnMobileOnly":
                            netverifySDK.setDataExtractionOnMobileOnly(config[category][param]);
                            break;
                        case "country":
                            netverifySDK.setPreselectedCountry(config[category][param]);
                            break;
                        case "documentVariant":
                            var NVDocumentVariant = nativeClasses.NVDocumentVariant;
                            var docVariant = null;
                            if ((config[category][param]).toUpperCase() == "PLASTIC") docVariant = NVDocumentVariant.PLASTIC;
                            else if ((config[category][param]).toUpperCase() == "PAPER") docVariant = NVDocumentVariant.PAPER;
                        ...
                        case "documentTypes":
                            ...
                            netverifySDK.setPreselectedDocumentTypes(documentTypeList);
                            break;
                        case "merchantScanReference":
                            netverifySDK.setMerchantScanReference(config[category][param]);
                            break;
                        case "merchantReportingCriteria":
                            netverifySDK.setMerchantReportingCriteria(config[category][param]);
                            break;
                        case "customerId":
                            netverifySDK.setCustomerId(config[category][param]);
                            break;
                        case "additionalInfo":
                            netverifySDK.setAdditionalInformation(config[category][param]);
                            break;
                        case "enableEMRTD":
                            netverifySDK.setEnableEMRTD(config[category][param]);
                            break;
                        case "setCameraPostion":
                            var JumioCameraPosition = nativeClasses.JumioCameraPosition;
                            var camPosition = null;
                            if ((config[category][param]).toUpperCase() == "BACK") camPosition = JumioCameraPosition.BACK;
                            else if ((config[category][param]).toUpperCase() == "FRONT") camPosition = JumioCameraPosition.FRONT;
                            netverifySDK.setCameraPosition(camPosition);
                            break;
                    }
                }
            }
