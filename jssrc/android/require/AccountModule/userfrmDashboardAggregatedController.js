define({
    ...
    mapDataToBar: function() {
        ...
        for (var p = 0; p < balanceBars.length; p++) {
            ...
            this.view[thisBar].animate(kony.ui.createAnimation({
                0: {
                    height: 0,
                    opacity: 0,
                    "stepConfig": {}
            ...
            this.view[thisChildLabel].animate(kony.ui.createAnimation({
                0: {
                    opacity: 0,
                    "stepConfig": {}
                },
    ...
    setAccountsInfo: function(internalAccounts, externalAccounts, bankName) {
            ...
            externalAccounts.forEach(function(record) {
                if (bankName == kony.i18n.getLocalizedString("kony.mb.common.allAccounts") || bankName == (record.BankName).toLowerCase().trim()) {
                    if (record.TypeDescription !== null && record.TypeDescription !== undefined) {
                        ...
                        if ((record.TypeDescription.toLowerCase().trim() === (configManager.getConstantValue('SAVINGS')).toLowerCase()) || (record.TypeDescription.toLowerCase().trim() === (configManager.getConstantValue('CHECKING')).toLowerCase())) {
                            typeBalance[record.TypeDescription] = kony.i18n.getLocalizedString("kony.mb.accdetails.availBal");
                        } else if ((record.TypeDescription.toLowerCase().trim() === (configManager.getConstantValue('DEPOSIT')).toLowerCase()) || (record.TypeDescription.toLowerCase().trim() === (configManager.getConstantValue('CREDITCARD')).toLowerCase())) {
                            typeBalance[record.TypeDescription] = kony.i18n.getLocalizedString("kony.mb.accdetails.currBal");
                        } else if ((record.TypeDescription.toLowerCase().trim() === (configManager.getConstantValue('LOAN')).toLowerCase()) || (record.TypeDescription.toLowerCase().trim() === (configManager.getConstantValue('MORTGAGE')).toLowerCase())) {
                        ...
                        if (record.TypeDescription.toLowerCase() === "savings" || record.TypeDescription.toLowerCase() === "checking" || record.TypeDescription.toLowerCase() === "current") {
                            totalBalance = totalBalance + parseFloat(record.AvailableBalance);
                            totalAsset = totalAsset + parseFloat(record.AvailableBalance);
                        }
                        if (record.TypeDescription.toLowerCase() === "mortgage" || record.TypeDescription.toLowerCase() === "loan") {
                            totalDebt = totalDebt + parseFloat(record.AvailableBalance);
                        }
                        record.AvailableBalance = formatUtility.formatAmountandAppendCurrencySymbol(record.AvailableBalance);
        ...
        for (var type in typesJS) {
            ...
            data.push([{
                "lblTypeName": typeVal + " Accounts (" + typesJS[type].length + ")",
