define(["AsyncManager/BusinessControllers/BusinessController", "dataFormatUtility"], function(AsyncManager, dataFormater) {
    ...
    Account_PresentationController.prototype.fetchAccountExternalTransactions = function(mainUser, userName, bankId, account, accountDetailsObj) {
        ...
        navMan.setCustomInfo("frmAccountDetails", selectedAccount);
        var transactionObj = applicationManager.getTransactionManager();
        var params = kony.mvc.Expression.and(kony.mvc.Expression.eq("main_user", mainUser), kony.mvc.Expression.eq("username", userName), kony.mvc.Expression.eq("bank_id", bankId), kony.mvc.Expression.eq("account", account));
    ...
    Account_PresentationController.prototype.navigateToAccountDetails = function(res) {
        ...
        for (var i = 0; i < selectedAccount.pendingTransactions.length; i++) {
            if (selectedAccount.pendingTransactions[i]["isScheduled"] === "true") var trandateobj = formatUtil.getDateObjectfromString(selectedAccount.pendingTransactions[i]["scheduledDate"], "YYYY-MM-DD");
            else var trandateobj = formatUtil.getDateObjectfromString(selectedAccount.pendingTransactions[i]["transactionDate"], "YYYY-MM-DD");
        ...
        for (var i = 0; i < selectedAccount.postedTransaction.length; i++) {
            if (selectedAccount.postedTransaction[i]["isScheduled"] === "true") var trandateobj = formatUtil.getDateObjectfromString(selectedAccount.postedTransaction[i]["scheduledDate"], "YYYY-MM-DD");
            else var trandateobj = formatUtil.getDateObjectfromString(selectedAccount.postedTransaction[i]["transactionDate"], "YYYY-MM-DD");
    ...
    Account_PresentationController.prototype.getTotalAvailableBalance = function(data) {
        ...
        for (i = 0; i < data.length; i++) {
            if (data[i].accountType == configManager.constants.SAVINGS || data[i].accountType == configManager.constants.CHECKING) totalBalance = totalBalance + parseInt(data[i]["availableBalance"]);
        }
    ...
    Account_PresentationController.prototype.presentationAccountsErr = function(err) {
        kony.print(err);
        applicationManager.getPresentationUtility().dismissLoadingScreen();
        if (error["isServerUnreachable"]) applicationManager.getPresentationInterruptHandler().showErrorMessage("postLogin", err);
    };
    ...
    Account_PresentationController.prototype.setEntryPoints = function(transactionType) {
        ...
        switch (transactionType) {
            ...
            case "ExternalTransfer":
                ...
                break;
            case "P2P":
                navMan.setEntryPoint("payaperson", "frmAccountDetails");
                break;
            case "Deposit":
                navMan.setEntryPoint("Deposit", "frmAccountDetails");
                break;
            case "Cardless":
                navMan.setEntryPoint("cancelCardlessTransaction", "frmAccountDetails");
                break;
            case "BillPay":
