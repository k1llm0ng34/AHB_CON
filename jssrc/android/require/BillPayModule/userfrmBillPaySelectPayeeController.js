    ...
    setSegmentData: function() {
        ...
        for (var j = 0; j < recentPayees.length; j++) {
            if (recentPayees[j]["eBillSupport"] === "true") {
                if (recentPayees[j]["EBillEnable"] === "1") recentPayees[j]["imgebill"] = {
                    "src": "ebill.png",
                    "isVisible": true
                };
                else recentPayees[j]["imgebill"] = {
                    "src": "ebillinactive.png",
                    "isVisible": true
                };
            } else {
        ...
        if (allPayees.length === 0 && recentPayees.length == 0) {
            ...
            if (recentPayees.length > 0) segmentData.push([{
                "lblHeader": applicationManager.getPresentationUtility().getStringFromi18n("kony.mb.Transfers.recentpayees", "Recent Payees")
            }, recentPayees]);
            if (allPayees.length > 0) segmentData.push([{
                "lblHeader": applicationManager.getPresentationUtility().getStringFromi18n("kony.mb.Transfers.allpayees")
            }, allPayees]);
