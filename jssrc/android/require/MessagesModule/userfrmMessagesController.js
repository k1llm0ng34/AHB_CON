define({
    ...
    setDataBasedOnSelectedTab: function() {
        ...
        if (messagesMod.presentationController.messageTabSelected === "INBOX") this.setDataToSegment(messageData.inboxRequestsDetails);
        else this.setDataToSegment(messageData.deleteRequestDetails);
    },
    ...
    animateInboxToDelete: function() {
        ...
        this.view.flxDraftIndicator.animate(kony.ui.createAnimation({
            0: {
                left: "0%",
                "stepConfig": {}
            },
    ...
    onSearchCancel: function() {
        ...
        var messageData = navManager.getCustomInfo("frmMessages");
        var  messagesMod = kony.mvc.MDAApplication.getSharedInstance().getModuleManager().getModule("MessagesModule");
        if (messagesMod.presentationController.messageTabSelected === "INBOX") scopeObj.setDataToSegment(messageData.inboxRequestsDetails);
        else scopeObj.setDataToSegment(messageData.deleteRequestDetails);
    },
    onSearchingMessages: function() {
        ...
        var requestsDetails = [];
        var  messagesMod = kony.mvc.MDAApplication.getSharedInstance().getModuleManager().getModule("MessagesModule");
        if (messagesMod.presentationController.messageTabSelected === "INBOX") requestsDetails = messageData.inboxRequestsDetails;
        else requestsDetails = messageData.deleteRequestDetails;
    ...
    setDataToSegment: function(segmentData) {
        ...
        for (var i = 0; i < segmentData.length; i++) {
            ...
            segmentData[i].lblDelete = {
                "text": "Delete"
            };
            segmentData[i].lblReply = {
                "text": "Reply"
            };
            segmentData[i].imgDelete = {
                "scr": "deleteicon.png"
            };
            segmentData[i].imgReply = {
                "scr": "replyicon.png"
            };
        }
        //     for (var i = 0; i < segmentData.length; i++) 
