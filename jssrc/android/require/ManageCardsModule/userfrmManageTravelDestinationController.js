define({
    ...
    setLocationsData: function() {
        ...
        try {
            ...
            loggerManager.log("frmManageTravelDestinationController: " + JSON.stringify(locs));
            if (!kony.sdk.isNullOrUndefined(locs) && !kony.sdk.isEmptyObject(locs)) {
                this.setCountryObject(locs.countries);
                this.setStatesObject(locs.states);
                this.setCitiesObject(locs.cities);
            } else {
    ...
    updateSelectedlist: function() {
        ...
        try {
            ...
            if (listLength === destinationLimit || alreadyAdded === true) {
                ...
                } else loggerManager.log("frmManageTravelDestination: Cannot add more data into the destinations crossed the limit" + destinationLimit);
            } else {
                var dataMap = {
                    "lblName": "destinationValue",
                    "imgIcon": "image"
    ...
    setCountryObject: function(countryList) {
        ...
        try {
            var navManager = applicationManager.getNavigationManager();
            var currentForm = navManager.getCurrentForm();
            loggerManager.log("#### setCountryObject #### for form: " + currentForm);
            var self = this;
    ...
    setStatesObject: function(stateList) {
        ...
        try {
            var navManager = applicationManager.getNavigationManager();
            var currentForm = navManager.getCurrentForm();
            loggerManager.log("#### setStatesObject #### for form: " + currentForm);
            var self = this;
    ...
    setCitiesObject: function(cityList) {
        ...
        try {
            var navManager = applicationManager.getNavigationManager();
            var currentForm = navManager.getCurrentForm();
            loggerManager.log("#### setCitiesObject #### for form: " + currentForm);
            var self = this;
    ...
    saveAndNavigate: function() {
        ...
        try {
            ...
            var destinations = [];
            updatedDestinationlist.forEach(function(element) {
