define({
    ...
    setTouchId: function(flags) {
        if (flags.isTouchIdSupported) this.view.flxOption2.isVisible = true;
        else this.view.flxOption2.isVisible = false;
        this.view.lblStatus2.text = kony.i18n.getLocalizedString("kony.mb.common.enabled");
    },
    setFaceId: function(flags) {
        var authMod = kony.mvc.MDAApplication.getSharedInstance().getModuleManager().getModule("AuthModule");
        if (authMod.presentationController.isGemaltoEnabledFlag == false) {
            this.view.flxOption4.isVisible = false;
            this.setNativeFaceId(flags);
        } else {
            if (flags.isFaceIdSupported) this.setNativeFaceId(flags);
            else {
                if (flags.isFaceIdEnrolled) this.view.lblStatus4.text = kony.i18n.getLocalizedString("kony.mb.common.enabled");
                else this.view.lblStatus4.text = "Enable";
    ...
    setNativeFaceId: function(flags) {
        if (flags.isFaceIdSupported) {
            ...
            } else this.view.flxOption4.isVisible = false;
        }
    },
    setPinLogin: function(flags) {
        if (flags.isPinEnabled) this.view.lblStatus3.text = kony.i18n.getLocalizedString("kony.mb.Generated");
        else this.view.lblStatus3.text = kony.i18n.getLocalizedString("kony.mb.devReg.generate");
    },
    ...
    flxFaceIdNavigation: function() {
        ...
        if (gemalto) {
            if (flagData.isFaceIdSupported && flagData.isFaceIdAvailable) this.navigationForFaceIdDefault();
            else {
                if (flagData.isFaceIdEnrolled) this.navigationForFaceIdDefault();
                else {
