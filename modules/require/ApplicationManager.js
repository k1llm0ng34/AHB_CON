...
{
  var fabricData = getApplicationFabricBuildConfiguration();
  applicationManager.initWithServiceDoc(fabricData.appKey, fabricData.appSecret, fabricData.serviceDoc);
...
ApplicationManager.prototype.postAppInitiate = function()
{
 ...
 // navManger.stack.push("frmLogin");
  navManger.stack.push("frmAHBLanding");
  this.getDataforLogin();
  //getDeviceLocale();
 // authMode.presentationController.fetchPreloginAds();
 authMode.presentationController.fetchConfiguration(); 
};

ApplicationManager.prototype.initWithServiceDoc = function(appKey, appSecret, serviceDoc){
  function successCallback (result){


  }

  function failureCallback(errData){
    var err = {errorMessage : kony.i18n.getLocalizedString("kony.error.App_Initialisation_Failed")};
  ...
  try{
	...
	var data = serviceDoc;
    var processServiceDocResult = KNYMobileFabric.initWithServiceDoc(appKey, appSecret, data);
    if (processServiceDocResult === true) {
      ...
      kony.sdk.logsdk.debug("### init::_doInit saving done. Calling success callback",data);
          if(KNYMobileFabric)
      			KNYMetricsService = KNYMobileFabric.metricsServiceObject;
      kony.sdk.initiateSession(KNYMobileFabric);
      ...
      identityObject.login(null,
        ...
        successCallback(data);
                           ...
                           function(res) {
        failureCallback (kony.sdk.error.getAuthErrObj(res));
      });
...
ApplicationManager.prototype.getDataforLogin = function()
{
  ...
  loginData.isMpinStored = userManager.isMpinStored();
  loginData.isCifIdStored = userManager.isCifIdStored(); 
  loginData.isDeviceRegistered = userManager.isDeviceRegistered(); 
  loginData.cifId = sm.getStoredEncryptedItem("cifId");
  loginData.Mpin = sm.getStoredEncryptedItem("mPin");
//   var nuoData=navManager.getCustomInfo("frmLogin");
//   if(nuoData && nuoData.NUOUsername){
//     loginData.NUOUsername=nuoData.NUOUsername;
//   }
  navManager.setCustomInfo("frmAHBLanding", loginData);//frmLogin
  return loginData;
};
