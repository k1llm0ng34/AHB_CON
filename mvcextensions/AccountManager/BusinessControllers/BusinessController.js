 ...
 */
AccountManager.prototype.fetchInternalAccounts = function(cifObj, presentationSuccessCallback, presentationErrorCallback) {
    var self = this;
  	kony.print("cifObj"+cifObj);
    var accountsRepo = kony.mvc.MDAApplication.getSharedInstance().getRepoManager().getRepository("Accounts");
    accountsRepo.customVerb('getAccountsDashboard', cifObj, getAllCompletionCallback);
...
AccountManager.prototype.fetchAccountStatments = function(criteria, presentationSuccessCallback, presentationErrorCallback) {
    ...
    var accStatements = kony.mvc.MDAApplication.getSharedInstance().getRepoManager().getRepository("Accounts");
    accStatements.customVerb("getAccountStatements", criteria, getAllCompletionCallback);
 ...
 * @param {Function} presentationErrorCallback - will be called when call is not successful.
 */
  AccountManager.prototype.fetchAccountsIntermediateDetail = function(records,presentationSuccess,presentationError){
    ...
    var  accountsObj  =  kony.mvc.MDAApplication.getSharedInstance().getRepoManager().getRepository("Accounts");
     accountsObj.customVerb('getAccountDetails',records,getAllCompletionCallback);
    function  getAllCompletionCallback(status,  data,  error) {
    var srh = applicationManager.getServiceResponseHandler();
    var obj =  srh.manageResponse(status,  data,  error);
    if(obj["status"] === true){
   ...
   AccountManager.prototype.getLinkedDebitCards = function(record, presentationSuccessCallback, presentationErrorCallback) {
    var self = this;
    var accountsRepo = kony.mvc.MDAApplication.getSharedInstance().getRepoManager().getRepository("Accounts");
    accountsRepo.customVerb('getDebitCards', record , getAllCompletionCallback);
    kony.print("BC : getLinkedDebitCards");

    function getAllCompletionCallback(status, data, error) {
        var srh = applicationManager.getServiceResponseHandler();
        var obj = srh.manageResponse(status, data, error);
        kony.print("BC : getLinkedDebitCards gac ");
      if (obj["status"] === true) {
            presentationSuccessCallback(obj["data"]);
        } else {
            presentationErrorCallback(obj["errmsg"]);
  ...
  AccountManager.prototype.fetchAccountsDetailsByIBANOrSwiftCode = function(record, presentationSuccessCallback, presentationErrorCallback){

    var self = this;
    var accountsRepo = kony.mvc.MDAApplication.getSharedInstance().getRepoManager().getRepository("ExternalAccounts");
    accountsRepo.customVerb('getBankDetails', record , getAllCompletionCallback);
    kony.print("BC : fetchAccountsDetailsByIBANOrSwiftCode");

    function getAllCompletionCallback(status, data, error) {
      var srh = applicationManager.getServiceResponseHandler();
      var obj = srh.manageResponse(status, data, error);
      kony.print("BC : fetchAccountsDetailsByIBANOrSwiftCode  getAllCompletionCallback " + JSON.stringify(obj));
      if (obj["status"] === true) {
        presentationSuccessCallback(obj["data"]);
      } else {
        presentationErrorCallback(obj["errmsg"]);
  ...
  AccountManager.prototype.addBeneficiaryData = function(record, presentationSuccessCallback, presentationErrorCallback){

    var self = this;
    var accountsRepo = kony.mvc.MDAApplication.getSharedInstance().getRepoManager().getRepository("ExternalAccounts");
    accountsRepo.customVerb('addBeneficiary', record , getAllCompletionCallback);
    kony.print("BC : addBeneficiaryData");

    function getAllCompletionCallback(status, data, error) {
      var srh = applicationManager.getServiceResponseHandler();
      var obj = srh.manageResponse(status, data, error);
      kony.print("BC : addBeneficiaryData  getAllCompletionCallback " + JSON.stringify(obj));
      if (obj["status"] === true) {
        presentationSuccessCallback(obj["data"]);
      } else {
        presentationErrorCallback(obj["errmsg"]);
  ...
  AccountManager.prototype.validateOtherAHBCard = function(record, presentationSuccessCallback, presentationErrorCallback){
    var self = this;
    var accountsRepo = kony.mvc.MDAApplication.getSharedInstance().getRepoManager().getRepository("Accounts");
    accountsRepo.customVerb('validateCard', record , getAllCompletionCallback);
    kony.print("BC : validateOtherAHBCard");
    function getAllCompletionCallback(status, data, error) {
      var srh = applicationManager.getServiceResponseHandler();
      var obj = srh.manageResponse(status, data, error);
      kony.print("BC : validateOtherAHBCard  getAllCompletionCallback " + JSON.stringify(obj));
      if (obj["status"] === true) {
        presentationSuccessCallback(obj["data"]);
      } else {
        presentationErrorCallback(obj["errmsg"]);
