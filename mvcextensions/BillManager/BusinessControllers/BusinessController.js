...
BillManager.prototype.fetchUserBillPayPendingTransactions = function(criteria,presentationSuccessCallback, presentationErrorCallback){
  ...
  function  getAllCompletionCallback(status,  data,  error) {
   ...
   /**
   * Verifies Consumer details added while adding a payee
   * @param {object} request 
   * @param {function} presentationSuccessCallback will be called when call is successfull
   * @param {function} presentationErrorCallback will be called when call is not successfull
   */
  BillManager.prototype.verifyConsumerNumberAddPayee = function(request,presentationSuccessCallback,presentationErrorCallback){
   kony.print("request==="+JSON.stringify(request));
    var  payeeRepo  =  kony.mvc.MDAApplication.getSharedInstance().getRepoManager().getRepository("Payee");
    payeeRepo.customVerb("validatePayee",request, getAllCompletionCallback);
    function  getAllCompletionCallback(status,  data,  error) {
      var srh = applicationManager.getServiceResponseHandler();
      var obj =  srh.manageResponse(status,  data,  error);
      if(obj["status"] === true){
