...
AuthManager.prototype.login = function(UsernamePasswordJSON,presentationSuccess,presentationError){
    var authParams = {
                      "cifId":UsernamePasswordJSON.cifId,
       				  "mpin": UsernamePasswordJSON.mpin,
        			  "deviceId":UsernamePasswordJSON.deviceId,
                      "loginOptions": {
...
AuthManager.prototype.logout = function(presentationSuccess,presentationError){
   var self = this;
    var authLogout = KNYMobileFabric.getIntegrationService("AHBLoginService");
        var authParams = {
            "loginOptions": {
                "isOfflineEnabled": false
            }
        ...
        try {
      authLogout.invokeOperation("logout",{},authParams,logoutSuccessCallback, logoutErrorCallback);
    } catch (err) {
...
AuthManager.prototype.fetchOTP = function(requestOTPJSON,presentationSuccessCallback,presentationErrorCallback){
 var userRepo  =  kony.mvc.MDAApplication.getSharedInstance().getRepoManager().getRepository("User");
  ...
  userRepo.customVerb('requestOTP',params,getAllCompletionCallback);
...
AuthManager.prototype.verifyOTP = function(verifyOTPJSON,presentationSuccessCallback,presentationErrorCallback){
 var userRepo  =  kony.mvc.MDAApplication.getSharedInstance().getRepoManager().getRepository("User");
  ...
  userRepo.customVerb('verifyOTP',params,getAllCompletionCallback);
  function  getAllCompletionCallback(status,  data,  error) {
    var srh = applicationManager.getServiceResponseHandler();
    var obj =  srh.manageResponse(status,  data,  error,presentationSuccessCallback,presentationErrorCallback);
    if(obj["status"] === true){
...
AuthManager.prototype.resetPassword = function(resetPasswordJSON,presentationSuccessCallback,presentationErrorCallback){
 ...
 var params = resetPasswordJSON;
  userRepo.customVerb('resetPassword',params,getAllCompletionCallback);
  function  getAllCompletionCallback(status,  data,  error) {
    var srh = applicationManager.getServiceResponseHandler();
    var obj =  srh.manageResponse(status,  data,  error,presentationSuccessCallback,presentationErrorCallback);
    if(obj["status"] === true){
...
/**
  ...
  */
  ...
  AuthManager.prototype.fetchConfigurationData = function(data,presentationSuccessCallback,presentationErrorCallback){
    var userRepo  =  kony.mvc.MDAApplication.getSharedInstance().getRepoManager().getRepository("User");
    var params = data;
    userRepo.customVerb('getConfigurations',params,getAllCompletionCallback);
    function  getAllCompletionCallback(status,  data,  error) {
      var srh = applicationManager.getServiceResponseHandler();
      var obj =  srh.manageResponse(status,  data,  error,presentationSuccessCallback,presentationErrorCallback);
      if(obj["status"] === true){
  ...
  AuthManager.prototype.triggerOTPForAddBeneficiary = function(presentationSuccessCallback,presentationErrorCallback){
    var userRepo  =  kony.mvc.MDAApplication.getSharedInstance().getRepoManager().getRepository("User");
    var params = {};
    kony.print("triggerOTPForAddBeneficiary");
    userRepo.customVerb('triggerOTPPostLogin',params,getAllCompletionCallback);
    function  getAllCompletionCallback(status,  data,  error) {
      var srh = applicationManager.getServiceResponseHandler();
      var obj =  srh.manageResponse(status,  data,  error,presentationSuccessCallback,presentationErrorCallback);
      if(obj["status"] === true){
  ...
  AuthManager.prototype.resetMoblePin = function(data,presentationSuccessCallback,presentationErrorCallback){
   kony.print("entered resetMoblePin=="+JSON.stringify(data));
   var userRepo  =  kony.mvc.MDAApplication.getSharedInstance().getRepoManager().getRepository("User");
    var params = data;
    userRepo.customVerb('resetPin',params,getAllCompletionCallback);
    function  getAllCompletionCallback(status,  data,  error) {
      var srh = applicationManager.getServiceResponseHandler();
      var obj =  srh.manageResponse(status,  data,  error,presentationSuccessCallback,presentationErrorCallback);
      if(obj["status"] === true){
