...
/**
  ...
  */
userPreferencesManager.prototype.clearUserData = function(){
  var sm=applicationManager.getStorageManager();
...
userPreferencesManager.prototype.activateP2P=function(presentationSuccess, presentationFailure){
   ...
   /**
  * Function to validate credit card details wjile device registration using a service call.
  * @param {record} record - record consist of credit card number and credit card pin which has to be validated.
  ...
  */
userPreferencesManager.prototype.validateCreditCardDetails = function(record,presentationSuccess,presentationError)
{
  kony.print("validate credit card details");
     var  userObj  =  kony.mvc.MDAApplication.getSharedInstance().getRepoManager().getRepository("User");
     userObj.customVerb('verifyCVV',record,getAllCompletionCallback);
    function  getAllCompletionCallback(status,  data,  error) {
    var srh = applicationManager.getServiceResponseHandler();
    var obj =  srh.manageResponse(status,  data,  error);
    if(obj["status"] === true){
  ...
  /**
  * Function to validate credit card OTP while device registration using a service call.
  * @param {record} record - record consist of credit card number and credit card pin which has to be validated.
  * @param {callBack} presentationSuccessCallback - invoke the call back with success response.
  * @param {callBack} presentationErrorCallback - invoke the call back with error response.
  */
  userPreferencesManager.prototype.validateOTPcreditCard =  function(record,presentationSuccess,presentationError)
  {
    kony.print("validate credit card otp");
    var  userObj  =  kony.mvc.MDAApplication.getSharedInstance().getRepoManager().getRepository("User");
    userObj.customVerb('verifyOTP',record,getAllCompletionCallback);
    function  getAllCompletionCallback(status,  data,  error) {
      var srh = applicationManager.getServiceResponseHandler();
      var obj =  srh.manageResponse(status,  data,  error);
      if(obj["status"] === true){
    ...
    /**
  * Function to Set mobile pin .
  * @param {record} record - record consist of referenceKey and mpin.
  * @param {callBack} presentationSuccessCallback - invoke the call back with success response.
  * @param {callBack} presentationErrorCallback - invoke the call back with error response.
  */
userPreferencesManager.prototype.createMpin = function(record,presentationSuccess,presentationError)
{
  kony.print(" Set mobile pin");
     var  userObj  =  kony.mvc.MDAApplication.getSharedInstance().getRepoManager().getRepository("User");
     userObj.customVerb('create',record,getAllCompletionCallback);
    function  getAllCompletionCallback(status,  data,  error) {
    var srh = applicationManager.getServiceResponseHandler();
    var obj =  srh.manageResponse(status,  data,  error);
   ...
   /**
  * Gives info about Mpin stored 
  * @returns {boolean} - whether Mpin is stored/not.
  */
  
  userPreferencesManager.prototype.isMpinStored = function (){
  var sm=applicationManager.getStorageManager();
    if(sm.getStoredEncryptedItem("mPin")=== null || sm.getStoredEncryptedItem("mPin") === "")
    {  kony.print("sm"+JSON.stringify(sm))
    return false;
    }
  else{
     kony.print("sm"+JSON.stringify(sm))
  ...
  /**
  * Gives info about CifId stored 
  * @returns {boolean} - whether CifId is stored/not.
  */
  
  userPreferencesManager.prototype.isCifIdStored = function (){
  var sm=applicationManager.getStorageManager();
    if(sm.getStoredEncryptedItem("cifId")=== null || sm.getStoredEncryptedItem("cifId") === "")
    {  kony.print("cifId"+JSON.stringify(sm))
    return false;
    }
  else{
     kony.print("cifId"+JSON.stringify(sm))
...
Name: 		resendOTPAuth
Purpose:	Makes the service call to resend the OTP
Author: 	Fariha
Date: 		24/9/2018
Input:		Json object {'referenceKey':refKey,'cifId':cifId},
			resendUserOTP Success Callback func,
            resendUserOTP Error Callback func
Return: 	None

------------------------------------------------------------------------------------*/
  
  userPreferencesManager.prototype.resendOTPAuth =  function(record,presentationSuccess,presentationError)
  {
    kony.print("resend credit card otp");
    
    var  userObj  =  kony.mvc.MDAApplication.getSharedInstance().getRepoManager().getRepository("User");
    userObj.customVerb('requestOTP',record,getAllCompletionCallback);
    function  getAllCompletionCallback(status,  data,  error) {
      var srh = applicationManager.getServiceResponseHandler();
      var obj =  srh.manageResponse(status,  data,  error);
      if(obj["status"] === true){
  ...
  */
  userPreferencesManager.prototype.changeApplicationLanguage = function(pLanguage, pSuccessCallback, pErrorCallback){ 
   var self = this;
    kony.i18n.setCurrentLocaleAsync(pLanguage, onLanguageSuccessCallback, onLanguageFailureCallback);

   function onLanguageSuccessCallback(){
      self.updateUserFlag("languagePreference",pLanguage);
      var navMan = applicationManager.getNavigationManager();
      var entry = navMan.getEntryPoint("ChangeLanguage")
      //To check the entry before custom destroying forms
      if(entry === "frmLanguage"){
        navMan.customDestroyForms("frmLanguage");
      }else {
	  ...
	  if(pLanguage === "ar_AE"){
       kony.theme.setCurrentTheme("BlueThemeArabic",onThemeChangeSuccess,onThemeChangeSuccess);
     }else if(pLanguage === "en_US"){
       kony.theme.setCurrentTheme("BlueTheme",onThemeChangeSuccess,onThemeChangeSuccess);
     }else{
       kony.theme.setCurrentTheme("BlueTheme",onThemeChangeSuccess,onThemeChangeSuccess);
     }
     
     function onThemeChangeSuccess(){
       kony.print("Theme Changed!");
     }
      pSuccessCallback();
    };

    function onLanguageFailureCallback(){
  ...
  /**
  * get LanguagePrefrence
  * 
  */ 
  userPreferencesManager.prototype.getLanguagePrefrence = function()
  {
    var sm = applicationManager.getStorageManager();
    var value = sm.getStoredItem("settingsObject");
    var langPrefrences = value.languagePreference;
    return langPrefrences;  
  };
  
  /**
  * sets Default Application Language if languagePreference is not avaialbe
  * @param pCallback 
  * 
  */
  userPreferencesManager.prototype.setDefaultApplicationLanguage = function(pCallback){
    var self = this;
    
    var onDefaultLangSuccesscallback =  function(){     
      self.updateUserFlag("languagePreference",languagePreference); 
      pCallback();
    }
     var onThemeChangeSuccess = function(){
      kony.print("Theme changed!");
    }
    //read from languagePreference
    var sm = applicationManager.getStorageManager();
    var value = sm.getStoredItem("settingsObject");
    var languagePreference = value.languagePreference;
    ...
    if(languagePreference === null || languagePreference=== "" ){
      ...
      if(kony.os.deviceInfo().name ==="iPhone"){
        ...
        languagePreference = deviceLocale;
      }
      else{
        // android specific
        var lang = deviceLocale.language;
        var country = deviceLocale.country;
        languagePreference = lang+"_"+country;
        languagePreference = languagePreference.replace(/"/g, '');
      }
    }
    // finally you should have languagePreference as en_US / ar_AE
     if(languagePreference === "ar_AE"){
      kony.theme.setCurrentTheme("BlueThemeArabic",onThemeChangeSuccess,onThemeChangeSuccess);
    }
    if(languagePreference !== "ar_AE" && languagePreference!== "en_US" ){
      languagePreference = "en_US"
	  kony.theme.setCurrentTheme("BlueTheme",onThemeChangeSuccess,onThemeChangeSuccess);
    }

    kony.i18n.setCurrentLocaleAsync(languagePreference, onDefaultLangSuccesscallback, onDefaultLangSuccesscallback, null);
  }
