define(["AsyncManager/BusinessControllers/BusinessController"], function(AsyncManager) {
    ...
    //Payee data 
    scope_BillPayPresentationController.billPayType = "MULTI";
  ...
  //  if(scope_BillPayPresentationController.asyncManager.areAllservicesDone(2)){
    ...
    for(var i = 0;i<2;i++)
      ...
      for(var i = 0; i<bills.scheduledBills.length; i++){
        ...
        bills.scheduledBills[i]["scheduledDate"]= formatUtil.getFormatedDateString(billdateobj,formatUtil.APPLICATION_DATE_FORMAT)
        bills.scheduledBills[i]["showDate"] = bills.scheduledBills[i]["scheduledDate"];
        bills.scheduledBills[i]["amount"]=formatUtil.formatAmountandAppendCurrencySymbol(bills.scheduledBills[i]["amount"]);
      ...
      for (var i = 0; i < bills.pendingBills.length; i++) {
            ...
            bills.pendingBills[i]["transactionDate"] = formatUtil.getFormatedDateString(billdateobj, formatUtil.APPLICATION_DATE_FORMAT)
            ...
            bills.pendingBills[i]["amount"] = formatUtil.formatAmountandAppendCurrencySymbol(bills.pendingBills[i]["dueAmount"]);
            ...
            var paidAmount = formatUtil.formatAmountandAppendCurrencySymbol(bills.pendingBills[i]["paidAmount"]);
      ...
      for(var i = 0; i<bills.postedBills.length; i++){
        ...
        bills.postedBills[i]["transactionDate"]= formatUtil.getFormatedDateString(billdateobj,formatUtil.APPLICATION_DATE_FORMAT)
        bills.postedBills[i]["showDate"] = bills.postedBills[i]["transactionDate"];
        bills.postedBills[i]["amount"]=formatUtil.formatAmountandAppendCurrencySymbol(bills.postedBills[i]["amount"]);
 ...
 BillPay_PresentationController.prototype.payBill = function(payeeId){
  ...
  BillPay_PresentationController.prototype.getPayeeAddressDetails= function(){
   ...
   return recipientsManager.getBillPayPayeeData().reEnterAccountNumber;
    ...
    if(navManger.getEntryPoint("createBillPayPayee")==="frmBillPayAllPayees"){
      scope_BillPayPresentationController.fetchAllPayees();
    }
    else if(navManger.getEntryPoint("createBillPayPayee")==="frmBillPaySelectPayee")
    {
...
BillPay_PresentationController.prototype.isDefaultFromAccount = function(){
  ...
  BillPay_PresentationController.prototype.getAvailableBalanceCurrencyString = function(data)
  {
    ...
    {
      case configManager.constants.SAVINGS: return forUtility.formatAmountandAppendCurrencySymbol(data["availableBalance"]);
      case configManager.constants.CHECKING: return forUtility.formatAmountandAppendCurrencySymbol(data["availableBalance"]);  
      case configManager.constants.CREDITCARD: return forUtility.formatAmountandAppendCurrencySymbol(data["currentBalance"]);
      case configManager.constants.DEPOSIT: return forUtility.formatAmountandAppendCurrencySymbol(data["currentBalance"]);
      case configManager.constants.MORTGAGE: return forUtility.formatAmountandAppendCurrencySymbol(data["outstandingBalance"]);
      case configManager.constants.LOAN: return forUtility.formatAmountandAppendCurrencySymbol(data["outstandingBalance"]);
      default:return forUtility.formatAmountandAppendCurrencySymbol(data["availableBalance"]);
    }
  };
  ...
  BillPay_PresentationController.prototype.evaluateAmount = function(amount,fromAvlBal)
  {
    ...
    else
    {
      ...
      var navManger = applicationManager.getNavigationManager();
      //navManger.setCustomInfo("frmBillPayFrequency",{"index":index});
     ...
     for(var i=0;i<data.length;i++)
       {
            ...
            data[i]["scheduledDate"] = formatUtil.getFormatedDateString(billdateobj, formatUtil.APPLICATION_DATE_FORMAT);
