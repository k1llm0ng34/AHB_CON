...
define([], function() {
  ...
  function RecipientsManager(){
    ...
    this.listOfPayees = [];
     /**@member {Array} listOfScheduleds holds the list of Scheduled List retrieved information from the backend*/
    this.listOfScheduleds = [];
  ...
  */
  ...
  {
    var recepientsRepo  =  kony.mvc.MDAApplication.getSharedInstance().getRepoManager().getRepository("ExternalAccounts");
    recepientsRepo.customVerb('getBeneficiaries',{},fetchAllExternalBenificiariesCompletionCallback);
   ...
   /**
   * Fetches the list of ScheduledTransactions
   * @param {function} presentationManageBenificiarySuccess will be called when call is successful
   * @param {function} presentationManageBenificiaryError will be called when call is not successful
   */
  RecipientsManager.prototype.fetchAllScheduledTransactions = function(presentationScheduledTransactionsSuccess,presentationScheduledTransactionsError,request ={})
  {
    var recepientsRepo  =  kony.mvc.MDAApplication.getSharedInstance().getRepoManager().getRepository("Transactions");
    recepientsRepo.customVerb('getUserScheduledTransactions',request,fetchScheduledTransactionsCompletionCallback);

    function fetchScheduledTransactionsCompletionCallback(status,  data,  error){
      var srh = applicationManager.getServiceResponseHandler();
      var obj =  srh.manageResponse(status,  data,  error);

      if(obj.status === true){
        scope_RecipientsManager.listOfScheduleds=obj.data;
        presentationScheduledTransactionsSuccess(obj.data);
      }

      else {
        presentationScheduledTransactionsError(obj.errmsg);
      }
    }
  };
  ...
  {
    var recepientsRepo  =  kony.mvc.MDAApplication.getSharedInstance().getRepoManager().getRepository("ExternalAccounts");
    recepientsRepo.customVerb('updateBeneficiary',record,editBenificiaryCompletionCallback);
  ...
  {
    var recepientsRepo  =  kony.mvc.MDAApplication.getSharedInstance().getRepoManager().getRepository("ExternalAccounts");
    recepientsRepo.customVerb('deleteBeneficiary',record,deleteBenificiaryCompletionCallback);
  ...
  */
  ...
  {
    var getFreqSBA = kony.mvc.MDAApplication.getSharedInstance().getRepoManager().getRepository("ExternalAccounts");
    getFreqSBA.customVerb("getFrequentOtherBankAccount",{},getAllCompletionCallback);
    function getAllCompletionCallback(status, data, error) {
      var srh = applicationManager.getServiceResponseHandler();
      var obj = srh.manageResponse(status, data, error);
      if(obj.status === true){      
...
/**
  ...
  /**
   *  Method for adding a new payee
   * @param {Object} payeeDetails payee Data in a form of object
   * @param {function} presentationSuccessCallback will be called when call is successful
   * @param {function} presentationErrorCallback will be called when call is not successful
   */
  RecipientsManager.prototype.addNewPayee  = function(payeeDetails, presentationSuccessCallback,presentationErrorCallback){
    kony.print("addNewPayee=="+JSON.stringify(payeeDetails));
    var  payeeRepo  =  kony.mvc.MDAApplication.getSharedInstance().getRepoManager().getRepository("Payee");
    payeeRepo.customVerb("create",payeeDetails, getAllCompletionCallback);
    function  getAllCompletionCallback(status,  data,  error) {
      var srh = applicationManager.getServiceResponseHandler();
      var obj =  srh.manageResponse(status,  data,  error);
      if(obj["status"] === true){
  ...
  RecipientsManager.prototype.fetchBillPayees = function(paramDetails, fetchBillPayeesSuccess,fetchBillPayeesError)
  {
    var payeeModel = kony.mvc.MDAApplication.getSharedInstance().getRepoManager().getRepository("Payee");
    payeeModel.customVerb("getPayeeList",paramDetails,getAllCompletionCallback);
    function getAllCompletionCallback(status, data, error) {
      var srh = applicationManager.getServiceResponseHandler();
      var obj = srh.manageResponse(status, data, error);
      if(obj.status === true){      
        fetchBillPayeesSuccess(obj.data);
      }
      else {
        fetchBillPayeesError(obj.errmsg);
      }
    }
  };
