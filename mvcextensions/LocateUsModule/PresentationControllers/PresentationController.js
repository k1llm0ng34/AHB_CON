...
define([], function() {
  ...
  function geoLocationErrorCallBack(err) {
    ...
    var isIphone = deviceUtilManager.isIPhone();
     ...
     if (err.code == 2 && !isIphone) {
       ...
       controller.showLocationAlertPopUp();
     }
  ...
  */
  LocateUs_PresentationController.prototype.getNearByLocations = function(latLongObj){
    ...
    if(latLongObj.searchString){
            
            latLongObj.latitude = locateUsPresentationScope.currentLatitude;
            latLongObj.longitude = locateUsPresentationScope.currentLongitude
        }
  ...
  */
  LocateUs_PresentationController.prototype.getNearByLocationsSuccess = function(response){
    var formattedResponse = checkServiceResponseForErrorCode(response);
        ...
        if (formattedResponse.responseCode === "200" || formattedResponse.responseCode == "AHB_1023") {
            try {
                locateUsPresentationScope.logger.log("#### in success callback of getNearByLocations ####");
                var locationList = response.LocationsList;
                if (!locationList) {
                    locationList = [];
                }
                ...
                if (locationList.length === 0) {
                  var i18_noRecords = kony.i18n.getLocalizedString("i18n.maps.NoResultsFound");
                   	frmLocationController.bindGenericError(i18_noRecords)
                    frmLocationController.noRecordsFound();
                }
            } catch (e) {
        ...
        else if (formattedResponse.responseCode == "AHB_1081") {
            frmLocationController.bindGenericError(formattedResponse.message);
        }
    	locateUsPresentationScope.presentView();
        applicationManager.getPresentationUtility().dismissLoadingScreen();
};
  ...
  LocateUs_PresentationController.prototype.ProcessDataForView = function(locationList){
    ...
    try{
      ...
      for(var i=0;i<locationList.length;i++){
        var locImages =locateUsPresentationScope.getMapPinIcon(locationList[i].type);
        tempLocaterData =
          {
          lat: locationList[i].latitude,
          lon:locationList[i].longitude,
          name  :locationList[i].locationName ,//informationTitle,//addressLine1
          desc  :locationList[i].locationDescription,//addressLine1,//addressLine2
          addressLine1 : locationList[i].addressLine1  ,
          addressLine2:locationList[i].addressLine2,
          locationId : locationList[i].locationId,
          image: locImages.unselectImg,
          locImages:locImages,
  ...
  */
  LocateUs_PresentationController.prototype.getMapPinIcon = function(type){
    if (type == "ATM" || type.toLowerCase() === "atm") {
      return {unselectImg:"mappinatmunselected.png", selectImg:"mappinatmselected.png"};
    } else if (type == "BRANCH" || type == "MainBranch" || type.toLowerCase() === "branch") {
      return {unselectImg:"mappinbankunselected.png",selectImg:"mappinbankselected.png"};
    }
    else if(type === "CCDM" || type.toLowerCase() === "ccdm"){
      return {selectImg: "mappincashselected.png" , unselectImg: "mappincashunselected.png"};
    }
    else if(type === "currentLocation"){
      return "current_location.png";
    }
    else{
      return {selectImg: "location.png" , unselectImg: "location.png"};
    }
  };
